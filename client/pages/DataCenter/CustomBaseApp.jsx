...
import Viewer from "forge-dataviz-iot-react-components/client/components/Viewer.jsx";
import CustomToolTip from "./CustomToolTip.jsx";
//import DataPanelContainer from "forge-dataviz-iot-react-components/client/components/DataPanelContainer.jsx";
import DataPanelContainer from "./DataPanelContainer.jsx"
import HyperionToolContainer from "./HyperionToolContainer.jsx";
import { SpriteSize, SensorStyleDefinitions, PropIdGradientMap, PropertyIconMap } from "forge-dataviz-iot-react-components/client/config/SensorStyles.js";
import adskLogoSvg from "forge-dataviz-iot-react-components/assets/images/autodesk-logo.svg";
import ChronosTimeSlider from "./ChronosTimeSlider.jsx";
import BasicDatePicker from "forge-dataviz-iot-react-components/client/components/BasicDatePicker.jsx";
import { getPaddedRange, getTimeInEpochSeconds, getClosestValue, clamp } from "forge-dataviz-iot-react-components/shared/Utility";
import HeatmapOptions from "./HeatmapOptions.jsx";
import EventTypes from "forge-dataviz-iot-react-components/client/components/EventTypes.js";
...
export default function BaseApp(props) {
    ...
    const canDelete = useRef({});
    const setDeviceStatus = useRef(false)
    const setPopUPActive = useRef(false)
    ...
    const [firstdayOptions, setfirstdayOptions] = useState(null);
    const [lastdayOptions, setlastdayOptions] = useState(null);
    ...
    useEffect(() => {
        if(firstdayOptions && firstdayOptions){
            setTimeOptions(prevState => ({
                ...prevState,
                startTime:firstdayOptions,
                endTime:lastdayOptions,
            }))  
        }
    },[firstdayOptions, lastdayOptions])
    /**
    ...
    async function onDataReady() {
        ...
        async function generateViewables(shadingData) {
            ...
            for (let i = 0; i < leafNodes.length; i++) {
                ...
                for (let j = 0; leaf.shadingPoints && j < leaf.shadingPoints.length; j++) {
                    ...
                    const viewable = new DataVizCore.SpriteViewable(device.position, style, spriteDbId.current);
                    dbId2DeviceIdMap[spriteDbId.current] = device.id;
                    deviceId2DbIdMap[device.id] = spriteDbId.current;
                    deviceStatus[spriteDbId.current] = 1;
                    position[spriteDbId.current] = device.position;
                    spriteDbId.current=spriteDbId.current+1;
                    viewableData.addViewable(viewable);
                }
            }

            // Device with status == 0, 2
            const devices =  session.dataStore.deviceModels[0].devices
            for (let i = 0; i <devices.length; i++){
                if(devices[i].status==0){
                    ...
                    const viewable = new DataVizCore.SpriteViewable(devices[i].position,style , spriteDbId.current);
                    dbId2DeviceIdMap[spriteDbId.current] = devices[i].number;
                    deviceId2DbIdMap[devices[i].id] = spriteDbId.current;
                    deviceStatus[spriteDbId.current] = 0;
                    position[spriteDbId.current] = devices[i].position;
                    spriteDbId.current=spriteDbId.current+1;
                    viewableData.addViewable(viewable);
                }
            }
        ...
        async function editSprite(event){
            const adapter =session.dataStore.adapters[0]
                ...
                await fetch(url,{
                        ...
                        body: JSON.stringify({"id":device.id}) // body data type must match "Content-Type" header
        ...
        async function onItemClick(event) {
            ...
            if(canDelete.current.status){
                // for delete sprite node and set status = non active
                const deviceId = currAppState.dbId2DeviceIdMap[event.dbId];
                const device = currAppState.session.dataStore.getDevice(deviceId);
                ...
                canDelete.current = {
                    id:device.id,
                    status:canDelete.current.status,
                    adapter:adapter,
                    anchor:{
                        clientHeight: 0,
                        clientWidth: 0,
                        getBoundingClientRect: () => ({
                            top: mappedPosition.y + vertificalOffset - SpriteSize / viewer.getWindow().devicePixelRatio,
                            left: mappedPosition.x,
                            right: mappedPosition.x,
                            bottom: mappedPosition.y + vertificalOffset - SpriteSize / viewer.getWindow().devicePixelRatio,
                            width: 0,
                            height: 0,
                        })     
                    }   
                }
                setDeviceStatus.current = device.status;
                setPopUPActive.current = true;
            }
    ...
    async function createNewSprite  (_position) {
        const [session1, masterDataView] = await initializeDataStore();
        //setAppState({...appState,session:session})
        // console.log("change")
        // console.log(session1)
        // const dataVizExtn = await currAppState.viewer.loadExtension("Autodesk.DataVisualization");
        // dataVizExtn.removeAllViewables();
        // const DataVizCore = Autodesk.DataVisualization.Core;
        // const viewableData = new DataVizCore.ViewableData();
        // viewableData.spriteSize = props.surfaceShadingConfig && props.surfaceShadingConfig.spriteSize ? props.surfaceShadingConfig.spriteSize : SpriteSize;
        // for(let i = 1; i < spriteDbId.current;i ++){
        //     if(appState.deviceStatus[i]){
        //         // const style = getViewableStyle(device.contextData.styleId);
        //         console.log("here "+i)
        //         const viewableType = DataVizCore.ViewableType.SPRITE;
        //         const spriteColor = new THREE.Color(0xffffff);
        ...
        //         const viewable = new DataVizCore.SpriteViewable(appState.position[i], style, i);
        //         viewableData.addViewable(viewable);
        //     }else{
        //         const viewableType = DataVizCore.ViewableType.SPRITE;
        //         const spriteColor = new THREE.Color(0xffffff);
        ...
        // const spriteIconUrl = `${baseURL}fan-00.svg`;
        // const style = new DataVizCore.ViewableStyle(viewableType, spriteColor, spriteIconUrl);
        // const viewable = new DataVizCore.SpriteViewable(_position,style , spriteDbId.current);
        // viewableData.addViewable(viewable);
        // await viewableData.finish();
        // dataVizExtn.addViewables(viewableData);
        // dataVizExtn.showHideViewables(renderSettings.showViewables, renderSettings.occlusion);
    ...
    function handleTimeRangeUpdated(startTime, endTime, currentTime) {
        const currAppState = appStateRef.current;
        // if(firstdayOptions && lastdayOptions){
        //     const st = new Date(startTime.getTime()).setHours(0, 0, 0, 0);
        //     const st2 = new Date(firstdayOptions.getTime()).setHours(0, 0, 0, 0) 
        //     const et =  new Date(endTime.getTime()).setHours(0, 0, 0, 0) 
        //     const et2 = new Date(lastdayOptions.getTime()).setHours(0, 0, 0, 0) 
        //     // adjust et and et2 
        //     if (currAppState && currAppState.masterDataView  &&  st >= st2 && endTime >= startTime) {
        ...
        //     }
        // }else{
            if (currAppState && currAppState.masterDataView  &&  startTime > firstdayOptions  && endTime >= startTime) {
                setTimeWindow(startTime, endTime, currAppState.masterDataView);
        ...
        if (currAppState && options) {
            ...
            if (prop) {
                ...
                if (av) {
                    ...
                    let normalized = (value - av.minValues) / (av.maxValues - av.minValues);
    ...
    function getPropertyRanges(propertyId) {
        ...
        if (propertyId !== "None") {
            ...
            if(deviceProperty!=undefined){
                let startchange= new Date(0); // The 0 there is the key, which sets the date to the epoch
                startchange.setUTCSeconds(deviceProperty.firstDay);
                setfirstdayOptions(startchange)
                let endChange = new Date(0);
                endChange.setUTCSeconds(deviceProperty.lastDay);
                setlastdayOptions(endChange)
            }
            ...
            device.propIds.forEach((property) => {
                ...
                if (av) {
                    ...
                    av.tsValues.forEach((tsValue, index) => { 
                        seriesData.push({
                            value: [(tsValue-25200) * 1000, av.avgValues[index]],
                            label: {},
                        });
                    });
                    let deviceProperty = propertyMap.get(property);
