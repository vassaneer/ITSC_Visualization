...
export default function CustomViewer(props) {
    ...
    const viewerDomRef2 = useRef(null);
    ...
    function initializeViewer() {
        ...
        Autodesk.Viewing.Initializer(options, async function () {
            ...
            const viewer2 = new Autodesk.Viewing.GuiViewer3D(viewerDomRef2.current, {
                extensions: extensionsWithoutConfig,
                disabledExtensions: props.disabledExtensions || {},
            });
            ...
            extensionsWithConfig.forEach((ext) => {
                ...
                viewer2.loadExtension(ext, extensionsToLoad[ext])
            })
            ...
            const startedCode2 = viewer2.start(undefined, undefined, undefined, undefined, options);
            if (startedCode > 0 & startedCode2 > 0) {
                console.error("Failed to create a Viewer: WebGL not supported.");
                return;
            }

            const av = Autodesk.Viewing;
            var viewer1CameraChangeMutex;
            var viewer2CameraChangeMutex;
            viewer.addEventListener(av.GEOMETRY_LOADED_EVENT, onModelLoaded, { once: true });
            viewer2.addEventListener(av.GEOMETRY_LOADED_EVENT, onModelLoaded, { once: true });
            viewer.addEventListener(av.CAMERA_CHANGE_EVENT, function(){
            ...
            // viewer2.addEventListener(av.CAMERA_CHANGE_EVENT, function(){
            //     //critical block to prevent endless chained loop of events 
            ...
            loadModel(viewer2, props.docUrn);

            if (props.onViewerInitialized) {
                props.onViewerInitialized(viewer);
                props.onViewerInitialized(viewer2);
            }
        });
    }
    ...
    return (
        <React.Fragment>
            <div id="MyViewerDiv1" ref={viewerDomRef}></div>
            <div id="MyViewerDiv2" ref={viewerDomRef2}></div>
        </React.Fragment>
    );
    
}

CustomViewer.displayName = "CustomViewer";
