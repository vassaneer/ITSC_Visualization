...
function HyperionToolContainer(props) {
    ...
    const [spButtonAnchor, setSPButtonAnchor] = useState(null); //SP = Sprite
    const [popUpNameAnchor, setPopUpNameAnchor] = useState(null); //popup
    const [deviceButtonAnchor, setDeviceButtonAnchor] = useState(null); // Device
    const [expandNodeId, setExpandNodeId] = useState("");
    const [selectedNodeId, setSelectedNodeId] = useState("");
    const [showSSD, setShowSSD] = useState(false); // SSD = SurfaceShadingData
    const [showSP, setShowSP] = useState(false); //SP = Sprite
    const [fromDeviceTree, setFromDeviceTree] = useState(false); // Device
    ...
    const [resNewDevice, setResNewDevice] = useState(null)
    const [resEditDevice, setResEditDevice] = useState(null)
    ...
    const toggleChecked = async (event)=>{
        console.log(event)
        // var changed = props.canDelete.current
        // changed.checked = ! changed.checked
        // props.canDelete.current = changed
        // console.log(props.canDelete.current)
        setToggle(!toggle);
    }
    ...
    const handleActive = async ()=>{
        ...
        const url = `http://localhost:9000/api/sprite/editStatus?provider=${adapter}&project=unused`;
        await fetch(url,{
            ...
            body: JSON.stringify({"id":props.canDelete.current.id,"status":toggle}) // body data type must match "Content-Type" header
            }).then((response) => response.json())
            .then(async (jsonData) => {
                setResEditDevice(jsonData)
            });
            }

    /**
     * Event when click add sprite
     * 
     */
    

    const handlePopUpName = async (event)=>{
        if(name!=""){
            setPopUPName(false)
            const adapter = props.adapter
            const url = `http://localhost:9000/api/sprite/add?provider=${adapter}&project=unused`;
            const data = newDevice
            await fetch(url,{
                    method: 'POST', // *GET, POST, PUT, DELETE, etc.
                    mode: 'cors', // no-cors, *cors, same-origin
                    headers: {
            ...
            .then(async (jsonData) => {
                setResNewDevice(jsonData);
                setPopUPName(true);
                // add new sprite from root component
                //const position = new THREE.Vector3(parseFloat(jsonData.pos_x), parseFloat(jsonData.pos_y), parseFloat(jsonData.pos_z));
                //location.href = "/app"
                //props.createNewSprite(position);
    ...
    useEffect(()=>{
        ...
        const ev = DataVizCore.MOUSE_CLICK
        const fn = async function(event){ 
            // console.log("Received sprite click", event.clickInfo);
            /**
             * Post data to api
             * @param {string} Name name of device
             * @param {string} Rack name of rack
             * @param {float} Position_x position x in world coordinate
             * @param {float} Position_y position y in world coordinate
             * @param {float} Position_z position z in world coordinate
             * @param {string} url post to api
             */
             //props.eventBus.removeEventListener(ev,fn,false )
             //setCanAdd(false)              
            if (event.clickInfo){
                setPopUPName(true);
                const name = await getRoomName(event.clickInfo.dbId, props.model)
                console.log(event.clickInfo.dbId)
                setNewDevice({
                    dbId:event.clickInfo.dbId,
                    pos_x:event.clickInfo.intersectPoint.x,
                    pos_y:event.clickInfo.intersectPoint.y,
                    pos_z:event.clickInfo.intersectPoint.z,
                    nameRack:name
                })
            }
        }
        const  handlePopUpClick = async function(event){
            setPopUpNameAnchor({
            clientHeight: 0,
            clientWidth: 0,
        ...
        if(canAdd){
            props.eventBus.addEventListener(ev,fn,true)
    ...
    useEffect(()=>{
        // when click on sprite
        setPopUPActive(props.setPopUPActive.current)
    },[props.setPopUPActive.current])

    useEffect(()=>{
        setToggle(props.setDeviceStatus.current)
    },[props.setDeviceStatus.current])
     ...
     * @private
     */
     const handleSPButtonClick = (event) => {
        setSPButtonAnchor(event.currentTarget);
        setShowSP(true);
        setShowSSD(false);
        setDeviceButtonAnchor(null)
    ...
    /**
     * Uses to add new sprite
     * 
     * @param {MouseEvent} event Click event indicating that the user has add the 
     * @private
     */
    ...
    const handleDeleteSprite = () => {
        props.canDelete.current={status:true};
        setCanAdd(false)
        setPopUPName(false)
    };
        ...
        <React.Fragment>
            <div id="hyperionToolContainer">
                ...
                {/* <IconButton
                    ...
                    ref={setSPButtonAnchor}
                >
                <AdjustIcon style={{ fill: "inherit" }} />
                </IconButton>
                <Popper
                   className={classes.popper}
                   open={showSP}
                   anchorEl={spButtonAnchor}
                   placement="left-start"
                   modifiers={{
                    flip: {
                        enabled: true,
                ...
                <React.Fragment>
                 ...
                 <ClickAwayListener onClickAway={handleSettingsClickAway}>
                    <Typography className={classes.typography} component="div">
                    <FormGroup className={classes.formGroupRoot}>
                      <FormLabel component="legend" className={classes.formLabel}>Sprite Config</FormLabel>
                      <div className={classes.grid}>
                      <IconButton 
                      className={classes.customHoverFocus}
                      onClick={handleAddSprite}
                      >
                          <AddCircleOutlineIcon style={{ fill: "inherit" }}></AddCircleOutlineIcon>
                      </IconButton>
                      <IconButton 
                      className={classes.customHoverFocus}
                      onClick={handleDeleteSprite}
                      >
                          <EditIcon style={{ fill: "inherit" }}></EditIcon>
                      </IconButton>
                      </div>
                    </FormGroup>
                    </Typography>
                ...
                {popUPName && (
                    ...
                    >
                      {resNewDevice==null && (
                          ...
                          <Typography className={classes.typography} component="div">
                          <FormGroup className={classes.formGroupRoot}>
                          <FormLabel component="legend" className={classes.formLabel}>Fill name of new device</FormLabel>                   
                              <TextField InputProps={{className: classes.formLabel}} label="Name" onChange={handleChangeName}/>
                              <div className={classes.divider}></div>
                              <Grid container item xs={12} spacing={2}>
                              <Grid item xs={6}>
                              <Button variant="contained" color="primary" onClick={handlePopUpName}>Submit</Button>
                              </Grid>
                              <Grid item xs={6}>
                              <Button variant="contained" color="primary" onClick={()=>setPopUPName(false)}>Cancel</Button>
                              </Grid>
                              </Grid>
                          </FormGroup>
                          </Typography>
                ...
                {resNewDevice!=null && (
                     ...
                     <Typography className={classes.typography} component="div">
                     <FormGroup className={classes.formGroupRoot}>
                        <FormLabel component="legend" className={classes.formLabel}>Status: {resNewDevice.status}</FormLabel> 
                        <div className={classes.divide} />
                        <FormLabel component="legend" className={classes.formLabel}>dbId: {resNewDevice.dbId}</FormLabel>                   
                        <div className={classes.divide} />
                        <FormLabel component="legend" className={classes.formLabel}>Name: {resNewDevice.device}</FormLabel> 
                        <div className={classes.divide} />
                        <FormLabel component="legend" className={classes.formLabel}>position_X: {resNewDevice.pos_x}</FormLabel> 
                        <div className={classes.divide} />
                        <FormLabel component="legend" className={classes.formLabel}>position_Y: {resNewDevice.pos_y}</FormLabel> 
                        <div className={classes.divide} />
                        <FormLabel component="legend" className={classes.formLabel}>position_Z: {resNewDevice.pos_z}</FormLabel> 
                        <div className={classes.divide} />
                        <Button variant="contained" color="primary" onClick={()=>{setPopUPName(false);setResNewDevice(null);window.location.reload(false);}}>Close</Button>
                     </FormGroup>
                     </Typography>
             </React.Fragment>
                )}
                ...
                { popUPActive && (
                    ...
                    className={classes.popper}
                    open={showSP}
                    anchorEl={props.canDelete.current.anchor}
                    placement="top"
                    >
                        {resEditDevice==null && (
                            <React.Fragment>
                            <Typography className={classes.typography} component="div">
                            <FormGroup className={classes.formGroupRoot}>
                            <FormLabel component="legend" className={classes.formLabel}>Device Status</FormLabel>                   
                            <FormControlLabel
                                control={<Switch size="small" checked={toggle} onChange={toggleChecked} />}
                                label="Active"
                            />
                            <div className={classes.divider}></div>
                              <Grid container item xs={12} spacing={2}>
                              <Grid item xs={6}>
                              <Button variant="contained" color="primary" onClick={handleActive}>Submit</Button>
                              </Grid>
                              <Grid item xs={6}>
                              <Button variant="contained" color="primary" onClick={()=>{
                                  setPopUPActive(false)
                                  props.setPopUPActive.current = false;
                                  }}>Cancel</Button>
                              </Grid>
                              </Grid>
                            </FormGroup>
                ...
                {resEditDevice!=null && (
                    ...
                    <Typography className={classes.typography} component="div">
                    <FormGroup className={classes.formGroupRoot}>
                        <FormLabel component="legend" className={classes.formLabel}>Status: {resEditDevice.status}</FormLabel> 
                        <div className={classes.divide} />
                        <Button variant="contained" color="primary" onClick={()=>{
                            setPopUPActive(false);
                            setResEditDevice(null);
                            window.location.reload(false);
                            }}>Close</Button>
                    </FormGroup>
                    </Typography>
